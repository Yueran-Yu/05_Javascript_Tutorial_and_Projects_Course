(function () {
  ...
  fieldForm.addEventListener('submit', function (e) {
    ...
    // 从 submit 表单后获取的值，放进容器，为feedback的显示做出准备
    const customer = new Customer(name.value, course.value, author.value)
    // console.log(customer)
    // 新建这个Dislay对象不是为了用里面的属性，而是为了调用挂载的方法
    const display = new Display()
  ...
  function Display() {
    this.name = fieldForm.querySelector('#name')
    this.course = fieldForm.querySelector('#course')
    this.author = fieldForm.querySelector('#author')
